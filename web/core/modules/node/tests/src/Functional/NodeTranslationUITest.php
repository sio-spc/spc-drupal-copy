      ...
      if ($translations[$langcode]->isPublished()) {
        ...
        foreach ($urls as $alternate_langcode => $language_url) {
          ...
          if ($translations[$alternate_langcode]->isPublished()) {
            ...
            $this->assertFalse(isset($links[0]), format_string('The %langcode node translation has an hreflang link for unpublished %alternate_langcode translation: %link.', ['%langcode' => $langcode, '%alternate_langcode' => $alternate_langcode, '%link' => $url->toString()]));
          }
        }
      }
    }
